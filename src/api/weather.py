...
from database import add_new_stats
from utils.excel import save_forecast_2_excel
from services.weather import weatherService as ws
...
async def get_weather_by_city_name(
    ...
    coordinates = ws.get_coordinates_by_city_name(city_name=city_name)
    if coordinates is None:
        add_new_stats(input_data=city_name, date=datetime.now(), status="Ошибка получения координат")
        return {"STATUS": "geocoder error"}

    raw_weather_data = ws.get_weather_forecast_by_lat_lon(coordinates["lat"], coordinates["lon"], days=days)
    ...
    weather_data = ws.get_weather_details(raw_weather_data)
